# SOTA-AI
探索全球顶级主流AI模型

| 产品 | 最新版本 | 厂家 | 类型 | 是否开源 | 链接 |
|------|----------|------|------|----------|------|
| ChatGPT | GPT‑5.5 | OpenAI | 基础 | 否 | https://chatgpt.com |
| Claude | Claude 4.7 Opus, Sonnet | Anthropic | 基础 | 否 | https://claude.ai |
| Gemini | Gemini 3.1 Pro, Flash | Google DeepMind | 基础 | 否 | https://gemini.google.com |
| Veo | Veo 3 | Google DeepMind | 视频生成 | 否 | https://deepmind.google/veo |
| Imagen | Imagen 4 | Google DeepMind | 图像生成 | 否 | https://deepmind.google/imagen |
| DeepSeek | DeepSeek‑V4 | DeepSeek | 基础 | 是 | https://chat.deepseek.com |
| 元宝 | Hy3 preview | 腾讯（混元团队） | 基础 | 否 | https://yuanbao.tencent.com |
| Kimi | K2.6 | 月之暗面（Moonshot AI） | 基础 | 是 | https://kimi.moonshot.cn |
| GLM | GLM‑5.1 | Z.ai（智谱AI） | 基础 | 部分开源 | https://z.ai / https://chatglm.cn |
| 豆包 | Doubao‑pro‑256k 等 | 字节跳动 | 基础 | 否 | https://doubao.com |
| MiniMax | MiniMax‑M2.7 | MiniMax | 基础 | 否 | https://minimax.io |
| 通义千问 (Qwen) | Qwen3.6 | 阿里云 | 基础 | 部分开源 | https://tongyi.aliyun.com |
| 通义万相 (Wan) | Wan2.7 | 阿里云 | 视频生成 | 部分开源 | https://wan.video |
| Grok | Grok‑4 | xAI | 基础 | 否 | https://x.ai |
| Midjourney | V7 | Midjourney | 图像生成 | 否 | https://midjourney.com |
| Stable Diffusion | 4.0 | Stability AI | 图像生成 | 是 | https://stability.ai |
| Stable Audio | 2.0 | Stability AI | 音乐/音频生成 | 是 | https://stability.ai |
| Suno | V4.5 | Suno AI | 音乐生成 | 否 | https://suno.com |
| Udio | v2 | Udio | 音乐生成 | 否 | https://udio.com |
| Runway Gen | Gen‑4 | Runway | 视频生成 | 否 | https://runwayml.com |
| Kling (可灵) | 2.0 | 快手 | 视频生成 | 否 | https://kling.kuaishou.com |
| Pika | 2.0 | Pika Labs | 视频生成 | 否 | https://pika.art |
| 即梦 | 3.0 | 字节跳动 | 图像/视频生成 | 否 | https://jimeng.jianying.com |
| Seedance | 2 | 字节跳动 | 视频生成 | 否 | https://seedance.com |
| Hailuo AI | MiniMax‑Video‑01 | MiniMax | 视频生成 | 否 | https://hailuoai.com |
| VoxCPM | VoxCPM2 | OpenBMB（面壁智能） | 语音合成 | 是 | https://github.com/OpenBMB/VoxCPM |
| Muse Spark | muse-spark | Meta | 基础 | 否 | https://meta.ai |
| MiMo | MiMo-V2-Pro | 小米 | 基础 | 否 | https://mimo.xiaomi.com/mimo-v2-pro |
| ChatGPT Images | 2.0 | OpenAI | 图像生成 | 否 | https://openai.com |
| Gemma | Gemma 4 | Google DeepMind | 基础 | 是 | https://ai.google.dev/gemma |
| Nano Banana | Nano Banana 2 | Google DeepMind | 图像生成 | 否 | https://aistudio.google.com / 使用 Gemini API |
| Grok Imagine | grok-imagine-image | xAI | 图像生成 | 否 |  https://grok.com/imagine |
| Flux | FLUX.2 | Black Forest Labs | 图像生成 | 部分开源 | https://github.com/black-forest-labs |
| Happy Horse | Happy Horse 1.0 | 阿里巴巴（未来光实验室） | 视频生成 | 是 | https://github.com（请以官方发布链接为准） |
